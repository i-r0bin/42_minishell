...
void output_redirection(t_data *data, int index) {
    ...
    if (check_file_permissions(data->args[index + 1], O_WRONLY | O_CREAT | O_TRUNC) < 0)
	{
		data->status = 1;
        return;
	}
...
void append_redirection(t_data *data, int index) {
    ...
    if (check_file_permissions(data->args[index + 1], O_WRONLY | O_CREAT | O_APPEND) < 0)
	{
		data->status = 1;
        return;
	}
...
void	exec_here_documents(t_data *data, int index)
{
	...
	while (1)
	{
		...
		if (ft_strncmp(line, data->args[index + 1], ft_strlen(data->args[index
					+ 1]) + 1) == 0)
		{
			data->line = ft_append_str(data->line, "\n");
			free(line);
